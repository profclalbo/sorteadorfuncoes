...
function reiniciar() {
  ...
  alert("Todos os dados foram reiniciados!"); // Mensagem de confirmação
}
...
function sortearFuncoes() {
    ...
    if (participantes.length === 4) {
      ...
      resultado.push(`${funcoes[0]} e ${funcoes[1]}: ${acumulador}`); // Monitor de Recursos e Harmonizador
      ...
      funcoes.forEach((funcao, index) => {
        if (index < sorteio.length) {
          if (funcao === "Repórter") {
            resultado.push(
              `<li class="reporter-highlight">${funcao}: ${sorteio[index]}</li>`
            ); // Adiciona a classe
          } else {
